...
class ChatConsumer(AsyncWebsocketConsumer):
    async def connect(self):
        ...
        if await self.get_user_count() < 2:
            logger.info(f"Пользователь {self.scope['user'].username} встал в чат {self.room_name}")
            ...
            logger.info(f"Пользователь {self.scope['user'].username} не встал в чат {self.room_name}")
            await self.close()
